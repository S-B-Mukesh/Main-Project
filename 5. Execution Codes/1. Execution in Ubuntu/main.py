    ...
    for attack_id in range(len(attacks)):
        ...
        for percentg in range(0, 6):    # percentage of malicious nodes
            ...
            for i in range(run_count):
                tcl_script = "wireless_jam_" + str(attacks[attack_id]) + "_" + str(mal_node_percentage) + "_" + str(i) + ".tcl"
                shutil.copyfile('wireless_jam.tcl',tcl_script)
                th_file_name = str(attacks[attack_id]) + "_" + out_file_names[0] + str(mal_node_percentage)
                de_file_name = str(attacks[attack_id]) + "_" + out_file_names[1] + str(mal_node_percentage)
                command = "ns " + tcl_script + " " + str(node_count) + " " + str(mal_node_percentage) + " " + attacks[attack_id] + " " + th_file_name + " " + de_file_name + " " + str(i) + " > /dev/null 2>&1"
        ...
        out_del_fh.close()
        out_thr_fh.close()
        print("Average Throughput Graph :\t" + str(out_file_names[0] + attacks[attack_id]))
        print("Average Delay Graph      :\t" + str(out_file_names[1] + attacks[attack_id]))
        print("\n===============================================================\n")

    print("Execution time :\t"+str(timedelta(seconds=time() - start_time)))
